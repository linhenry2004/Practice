    ...
    if len(indices) > 0:
        for i in indices.flatten():
            if (classes[classIDs[i]] == 'sports ball'):
                ...
                print(text)
                centerList.append(x + (w / 2))
                centerList.append(y + (h / 2))
                dimensionList.append(w)
                dimensionList.append(h)
                myList.append(centerList)
                myList.append(dimensionList)
...
def check_pos(prev, curr):
    ...
    cv.putText(img, text, (50, 50),
               cv.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), thickness=2)
...
KNOWN_DISTANCE = 30.0
KNOWN_WIDTH = 24.0
# This focalLength is only for size 8 basketballs
focalLength = 0.69
...
while(True):
    ...
    YOLO(img)
    myList.append(0)
    print('Markers = ', myList)
    if len(myList) == 3:
        cm = distance_to_camera(KNOWN_WIDTH, focalLength, myList[1][0])
        cm *= 1000
        tempList.append(myList[0][0])
        tempList.append(myList[0][1])
        tempList.append(round(cm, 2))
        posList.append(list(tempList))
        box = cv.cv.BoxPoints(
            myList) if imutils.is_cv2() else cv.boxPoints(myList)
        box = np.int0(box)
        ...
        print('Current Position: ', posList[-1])

    if len(posList) >= 2:
        check_pos(posList[-2], posList[-1])
    tempList.clear()
    myList.clear()
    cv.imshow("image", img)
    print('\n\n')
